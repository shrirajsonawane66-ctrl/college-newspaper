...
export default function LivePreview({
  ...
  const imageUrl = data.imageUrl || data.thumbnailUrl || data.coverImage || getArticleThumbnail(articleId);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[9px] uppercase tracking-[0.2em] text-emerald-600 font-sans font-semibold flex items-center gap-1">
      ...
      <div className="border border-border bg-paper overflow-hidden shadow-sm">
          ...
          {imageUrl ? (
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-[9px] text-ink-faded/50 mt-1 font-sans">No hero image</p>
              </div>
            </div>
          )}
          {data.category && (
            <div className="absolute top-2 left-2 bg-ink/80 text-paper text-[8px] uppercase tracking-[0.15em] px-2 py-0.5 font-sans font-semibold">
              {data.category}
            </div>
          )}
        </div>
        ...
        <div className="p-3 space-y-2">
          <h3 className="font-serif font-bold text-ink text-sm leading-snug line-clamp-2">
            {data.title || (
          ...
          {data.subheadline && (
            <p className="text-[11px] text-ink-lighter font-serif italic leading-relaxed line-clamp-1">
              {data.subheadline}
            </p>
          )}
          ...
          <p className="text-[11px] text-ink-light font-body leading-relaxed line-clamp-2">
            {data.summary || (
              <span className="text-ink-faded/30 italic">Summary&hellip;</span>
            )}
          </p>
          ...
          <div className="flex items-center gap-2 text-[9px] text-ink-faded font-sans border-t border-border pt-2">
            {data.author ? (
              <>
            ...
            {data.dropCap && (
              <>
                <span className="text-border">|</span>
                <span className="text-sepia">Drop cap</span>
              </>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="border border-border bg-paper-dark/30 p-2.5 text-center">
          ...
          <p className="text-[9px] uppercase tracking-[0.15em] text-ink-faded font-sans font-semibold">Words</p>
        </div>
        <div className="border border-border bg-paper-dark/30 p-2.5 text-center">
          <p className="text-lg font-bold font-serif text-ink tabular-nums">{charCount}</p>
          <p className="text-[9px] uppercase tracking-[0.15em] text-ink-faded font-sans font-semibold">Characters</p>
        </div>
        <div className="border border-border bg-paper-dark/30 p-2.5 text-center">
          <p className="text-lg font-bold font-serif text-ink tabular-nums">{data.readTime || "\u2014"}</p>
          <p className="text-[9px] uppercase tracking-[0.15em] text-ink-faded font-sans font-semibold">Read Time</p>
        </div>
        <div className="border border-border bg-paper-dark/30 p-2.5 text-center">
          <p className="text-lg font-bold font-serif text-ink tabular-nums">{data.publishedAt || "\u2014"}</p>
          <p className="text-[9px] uppercase tracking-[0.15em] text-ink-faded font-sans font-semibold">Published</p>
        </div>
      </div>
    </div>
  );
